...
pub fn get_flag<T: std::fmt::Display + serde::Serialize>(user: T) -> String {
    return full(user).results.attributes.blacklist_flag.unwrap_or("None".to_string())
}
...
pub fn get_original_ptid<T: std::fmt::Display + serde::Serialize>(user: T) -> String {
    return full(user).results.attributes.original_private_id.unwrap_or("None".to_string())
}
...
pub fn get_reason<T: std::fmt::Display + serde::Serialize>(user: T) -> String {
    return full(user).results.attributes.blacklist_reason.unwrap_or("None".to_string())
}
...
pub fn get_spam_predict<T: std::fmt::Display + serde::Serialize>(user: T) -> f64 {
    return full(user).results.spam_prediction.spam_prediction.unwrap_or(0.0)
}

pub fn get_ham_predict<T: std::fmt::Display + serde::Serialize>(user: T) -> f64 {
    return full(user).results.spam_prediction.ham_prediction.unwrap_or(0.0)
}
